class UsersController < ApplicationController
  ...
  def register
    ...
    if @user.save
      render json: {authMethod: 'register', user_id: @user.id.to_s, user_name: @user.user_name, user_email: @user.email, @user.book_count: "0", access_token: @user.access_token, group_name: @group_name}
    else
...
# POST /login
  def login
    ...
    if @user.nil?
      ...
      render json: {authMethod: 'login', user_id: @user.id.to_s, user_name: @user.user_name, user_email: @user.email, book_count: Book.where({user_id: @user.id.to_s}).count.to_s, access_token: @user.access_token, group_name: @user.group.group_name}
    end
  end
